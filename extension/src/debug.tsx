    ...
    <List isLoading={isLoading} searchBarPlaceholder="Search history...">
      {history.length === 0 && !isLoading && (
        <List.EmptyView
          ...
          description="Play some tracks to populate history."
        />
      )}
      {history.map((item, index) => (
        <List.Item
          ...
          accessories={[
            {
              icon: item.badgeUrl ? Icon.StarCircle : undefined,
              tooltip: item.badgeUrl ? "iTunes API Match" : "",
            },
          ]}
          actions={
            <ActionPanel>
              <Action
                title="Re-raise Notification"
                icon={Icon.AppWindowList}
                onAction={() => handleReRaise(item)}
              />
            </ActionPanel>
          }
        />
